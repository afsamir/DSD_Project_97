...
// Function Prototypes
static bool initialize_aws_miner(void *thr, int serial_fpga_num);
...
static inline void swap_endian_64(void *dest_p, const void *src_p, int bytes)
{
	...
	for (i = 0; i < num; i += 2) {
		dest[i] = swab32(src[i + 1]);
		dest[i + 1] = swab32(src[i]);
	}
}
...
void blakecoinmidstate(void *state, const void *input);
...
void decred_midstate(void *output, const void *input);
...
void nist5_midstate(void *output, const void *input);
void phi_hash(void *output, const void *input);
void phi_midstate(void *output, const void *input);
// void pluck_hash(uint32_t *hash, const uint32_t *data, uchar *hashbuffer, const int N);
void poly_hash(void *output, const void *input);
void poly_midstate(void *output, const void *input);
